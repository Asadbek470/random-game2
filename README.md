...
<body>
  ...
  <!-- Контейнер для игры Угадай число -->
  <div class="container">
    <header>
      <h1>Угадай число</h1>
  ...
  <script>
    ...
    window.onload = function() {
      // Загрузка данных для блокировки заметок
    ...
    async function checkGuess() {
      ...
      if (guess === targetNumber) {
        ...
        // Сохраняем данные (кроме администратора)
        if (currentUser !== './') {
          const saved = await saveData();
          if (saved) {
            updateLeaderboard();
